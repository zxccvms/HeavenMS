...
public class SearchCommand extends Command {
    ...
    public void execute(MapleClient c, String[] params) {
        ...
        if (params.length < 2) {
            player.yellowMessage("使用方法: !搜索 <分类> <名称>");
            return;
        }
        ...
        if (!params[0].equalsIgnoreCase("ITEM")) {
            ...
            if (params[0].equalsIgnoreCase("NPC")) {
                ...
                sb.append("#b搜索无效.\r\n使用方法: '!搜索 [分类] [名称]', 其中[分类]是地图、任务、NPC、物品、怪物或技能。");
            }
            if (data != null) {
                String name;
            ...
            for (Pair<Integer, String> itemPair : MapleItemInformationProvider.getInstance().getAllItems()) {
                if (sb.length() < 32654) {//ohlol
                    ...
                    sb.append("#b无法加载所有项，结果太多。\r\n");
                    break;
                }
            }
        }
        if (sb.length() == 0) {
            sb.append("#b").append(params[0].toLowerCase()).append("找不到.\r\n");
        }
        sb.append("\r\n#k载入使用：").append((double) (System.currentTimeMillis() - start) / 1000).append("秒.");//because I can, and it's free
