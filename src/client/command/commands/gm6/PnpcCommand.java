...
public class PnpcCommand extends Command {
    ...
    public void execute(MapleClient c, String[] params) {
        ...
        if (params.length < 1) {
            player.yellowMessage("使用方法: !设置npc <npcid>");
            return;
        }
        ...
        if (player.getMap().containsNPC(npcId)) {
            player.dropMessage(5, "此地图已包含指定的NPC.");
            return;
        }
        ...
        if (npc != null && !npc.getName().equals("MISSINGNO")) {
            try {
                ...
                player.yellowMessage("NPC已创建.");
            } catch (SQLException e) {
                e.printStackTrace();
                player.dropMessage(5, "无法将NPC存储在数据库中。");
            }
        } else {
            player.dropMessage(5, "您输入了无效的NPC id.");
        }
    }
}