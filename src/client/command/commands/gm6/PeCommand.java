...
public class PeCommand extends Command {
    ...
    public void execute(MapleClient c, String[] params) {
        ...
        try {
            ...
            ex.printStackTrace();
            player.yellowMessage("加载文本pe.txt失败");
            return;
        ...
        if (packetHandler != null && packetHandler.validateState(c)) {
            try {
                player.yellowMessage("接收: " + packet);
                packetHandler.handlePacket(slea, c);
            } catch (final Throwable t) {
                FilePrinter.printError(FilePrinter.PACKET_HANDLER + packetHandler.getClass().getName() + ".txt", t, "错误：" + (c.getPlayer() == null ? "" : "玩家 ; " + c.getPlayer() + "在地图 ; " + c.getPlayer().getMapId() + " - ") + "账号; " + c.getAccountName() + "\r\n" + slea.toString());
            }
        }
    }
}
